...
metadata:
  ...
  name: export-ubuntu                                                            # 导入后的虚拟机名称，可更改，同 namespace 下保持唯一
  namespace: default
spec:
  dataVolumeTemplates:
  - metadata:
      name: export-ubuntu-rootdisk                                               # 与下面 template spec 中的 volume 名称保持一致，同 namespace 下保持唯一
      namespace: default
    spec:
      pvc:
        accessModes:
  ...
  template:
    ...
    spec:
      ...
      volumes:
      - dataVolume:
          name: export-ubuntu-rootdisk                                        # 与上面 dataVolumeTemplates 中的名称保持一致
        name: rootdisk
```
